    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 30)
        ...
        if (GetOnscreenKeyboardResult()) then
           ...
           TriggerServerEvent('esx_pearls:annonce',result)
...
Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)    
                ...
                DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~y~ Pearls ~d~", 255, 255, 255, 255, 1, 0)
...
function setUniform(job, playerPed)
  TriggerEvent('skinchanger:getSkin', function(skin)
    ...
    if skin.sex == 0 then
      ...
      if job ~= 'citizen_wear' and job ~= 'pearls_outfit' then
...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    { label = _U('pearls_outfit'),    value = 'pearls_outfit'},
  }
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'pearls_outfit' then
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              'default', GetCurrentResourceName(), 'menu_crafting',
              {
                  ...
                  align = 'top-left',
                  elements = {
                      {label = _U('sushi'),     value = 'sushi'},
                      {label = _U('frites'),     value = 'fritesba'},
                  }
              },
              function(data2, menu2)
            
                TriggerServerEvent('esx_pearls:craftingBurger', data2.current.value)
          ...
          ESX.UI.Menu.Open(
              'default', GetCurrentResourceName(), 'menu_ingredient',
              {
                  ...
                  elements = {
                      ...
                      {label = _U('cuirepoisson'),     value = 'cuirepoisson'},
                      {label = _U('friteuse'),     value = 'friteuse'},
                  }
              },
              function(data2, menu3)
            
                TriggerServerEvent('esx_pearls:ingredientgBurger', data2.current.value)
                animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(player), 'society_pearls', _U('billing'), amount)
        end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0,  Keys['F6']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pearls_actions') then
...
RegisterNetEvent('esx_pearls:annonce')
AddEventHandler('esx_pearls:annonce', function(text)
    texteafiche = text
    affichenews = true
...
RegisterNetEvent('esx_pearls:annoncestop')
AddEventHandler('esx_pearls:annoncestop', function()
    affichenews = false
