...
Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)    
                ...
                DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~b~ EMS ~d~", 255, 255, 255, 255, 1, 0)
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_wear' then
        TriggerServerEvent("player:serviceOff", "ambulance")
			  ESX.ShowNotification("~r~~h~Fin~w~ de service~q~ Ambulancier~w~ !")
      ...
      if data.current.value == 'ambulance_wear' then
        TriggerServerEvent("player:serviceOn", "ambulance")
			  ESX.ShowNotification("~g~~h~Début~w~ de service ~q~Ambulancier~w~ !")
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
...
function OpenPharmacyMenu()
    ...
    ESX.ShowNotification("~b~~h~Vous avez pris un objet dans la ~g~Pharmacy~b~.")
