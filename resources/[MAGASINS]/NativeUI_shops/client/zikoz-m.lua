...
function AddShopsMenu(menu)
    ...
    local phone = NativeUI.CreateItem("Téléphone", "")
    gpsmenu.SubMenu:AddItem(phone)
    phone:RightLabel("~g~1500$")
	
	    local hifi = NativeUI.CreateItem("Hifi", "")
    gpsmenu.SubMenu:AddItem(hifi)
    hifi:RightLabel("~g~10$")
	
	  local donuts = NativeUI.CreateItem("donuts", "")
    shopsmenu2.SubMenu:AddItem(donuts)
    donuts:RightLabel("~g~10$")
	
	    local radio = NativeUI.CreateItem("Radio", "")
    gpsmenu.SubMenu:AddItem(radio)
    radio:RightLabel("~g~2000$")
    ...
    if item == donuts then
            TriggerServerEvent('buydonuts')
            ESX.ShowNotification('Vous avez payez ~r~10$')
            Citizen.Wait(1)
            ESX.ShowAdvancedNotification("Superette", "Vous avez acheter un  l'~b~donuts", "", "CHAR_LAZLOW2", 1)
   ...
   elseif item == pain then
    ...
    elseif item == phone then
        ...
        ESX.ShowAdvancedNotification("Superette", "Vous avez acheter un ~b~Téléphone", "", "CHAR_LAZLOW2", 1)
		    elseif item == hifi then
        TriggerServerEvent('buyhifi')
        ESX.ShowNotification('Vous avez payez ~r~10$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "Vous avez acheter un ~b~Hifi", "", "CHAR_LAZLOW2", 1)
		
		    --[[elseif item == donuts then
        TriggerServerEvent('buydonuts')
        ESX.ShowNotification('Vous avez payez ~r~10$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "Vous avez acheter un ~b~Donuts", "", "CHAR_LAZLOW2", 1)]]
		
		elseif item == radio then
        TriggerServerEvent('buyRadio')
        ESX.ShowNotification('Vous avez payez ~r~2000$')
        Citizen.Wait(1)
        ESX.ShowAdvancedNotification("Superette", "Vous avez acheter un ~b~Radio", "", "CHAR_LAZLOW2", 1)
...
local blips = {
    ...
    {title="Epicerie", colour=4, id=52, x = 4520.23, y = -4515.34,  z = 3.48},
	{title="Epicerie", colour=4, id=52, x = 377.95, y = 327.30, z = 103.56},
