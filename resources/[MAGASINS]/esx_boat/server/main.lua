...
function ParkBoats()
	MySQL.Async.execute('UPDATE owned_boat SET `stored` = true WHERE `stored` = false AND type = @type', {
		['@type'] = 'Boat'
...
ESX.RegisterServerCallback('esx_Boat:buyBoat', function(source, cb, vehicleProps)
	...
	if price == 0 then
		...
		if xPlayer.getMoney() >= price then
			...
			MySQL.Async.execute('INSERT INTO owned_boat (owner, plate, vehicle, type, `stored`) VALUES (@owner, @plate, @vehicle, @type, @stored)', {
...
AddEventHandler('esx_Boat:takeOutVehicle', function(plate)
	...
	MySQL.Async.execute('UPDATE owned_boat SET `stored` = @stored WHERE owner = @owner AND plate = @plate', {
		['@stored'] = false,
		['@owner']  = xPlayer.identifier,
		['@plate']  = plate
	}, function(rowsChanged)
...
ESX.RegisterServerCallback('esx_Boat:getGarage', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT vehicle FROM owned_boat WHERE owner = @owner AND type = @type AND `stored` = @stored', {
		['@owner']  = xPlayer.identifier,
		['@type']   = 'Boat',
		['@stored'] = true
	}, function(result)
