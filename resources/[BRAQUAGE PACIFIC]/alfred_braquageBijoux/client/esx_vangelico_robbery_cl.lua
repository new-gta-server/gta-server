...
AddEventHandler('esx_vangelico_robbery:VenteBijouxRefuser', function()
	PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
	Citizen.Wait(500)
	ESX.ShowAdvancedNotification('Revendeur', 'Revendeur de bijoux.', 'Hein ? Tu te fou de moi t\'a rien ?! Revient quand ta au minimum 10 bijoux!', 'CHAR_LESTER_DEATHWISH', 1)
	loopAlarm = false
	TriggerEvent('esx_vangelico_robbery:loopAlarmTriggered')
end)
...
local v1 = vector3(609.62, -459.62, 25.74)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "ta des bijoux l'ami ?")
        end
    end
end)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "g_m_m_chicold_01", 609.62, -459.62, 23.74, 135.15, false, true)
