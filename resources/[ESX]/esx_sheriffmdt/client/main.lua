...
RegisterNUICallback('esx_sheriffmdt:GetWarrantsClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:getWarrants', data.page)
end)

RegisterNUICallback('esx_sheriffmdt:GetBolosClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:getBolos', data.page)
end)

RegisterNUICallback('esx_sheriffmdt:GetArrestsClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:getArrests', data.page)
end)

RegisterNUICallback('esx_sheriffmdt:NewReportClient', function(data, cb)
	if data.report_type ~= '' and data.target ~= '' and data.desc ~= '' and data.charges ~= '' then
		TriggerServerEvent('esx_sheriffmdt:NewReportServer', data.report_type, data.target, data.desc, data.charges)
	end
end)

RegisterNUICallback('esx_sheriffmdt:CloseTab', function(data, cb)
	SendNUIMessage({
		type = 'GUI_pmobilet',
		StatusJS = false
	})
...
RegisterNUICallback('esx_sheriffmdt:SearchPersonClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:SearchPersonServer', data.fname, data.sname)
end)

RegisterNUICallback('esx_sheriffmdt:SearchVehicleClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:SearchVehicleServer', data.plate)
end)

RegisterNetEvent('esx_sheriffmdt:updateBolosList')
AddEventHandler('esx_sheriffmdt:updateBolosList', function(bolos_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
...
RegisterNetEvent('esx_sheriffmdt:updateArrestsList')
AddEventHandler('esx_sheriffmdt:updateArrestsList', function(arrests_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
	local design_shit = 'sui-row'
...
AddEventHandler('esx_sheriffmdt:NewReportResponse', function()
	SendNUIMessage({
		type = 'GUI_pmobilet',
		report_added = true
	})
end)
RegisterNetEvent('esx_sheriffmdt:NewReportResponse')

AddEventHandler('esx_sheriffmdt:SearchPersonFinish', function(player_name, properties, licenses, job, bills, has_warrants)
	local html_data = ''
...
RegisterNetEvent('esx_sheriffmdt:SearchPersonFinish')

AddEventHandler('esx_sheriffmdt:SearchVehicleFinish', function(veh_plate, veh_model, veh_owner)
	local html_data = ''
...
RegisterNetEvent('esx_sheriffmdt:SearchVehicleFinish')

AddEventHandler('esx_sheriffmdt:finishReportDelete', function(type_kek)
	if type_kek == 'warrants' then
		TriggerServerEvent('esx_sheriffmdt:getWarrants', 1)
	elseif type_kek == 'bolos' then
		TriggerServerEvent('esx_sheriffmdt:getBolos', 1)
	elseif type_kek == 'arrests' then
		TriggerServerEvent('esx_sheriffmdt:getArrests', 1)
	end
end)
RegisterNetEvent('esx_sheriffmdt:finishReportDelete')

RegisterNUICallback('esx_sheriffmdt:DeleteReportClient', function(data, cb)
	TriggerServerEvent('esx_sheriffmdt:DeleteReportServer', data.id, data.type)
end)

AddEventHandler('esx_sheriffmdt:updateWarrantsList', function(warrants_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
	local design_shit = 'sui-row'
...
RegisterNetEvent('esx_sheriffmdt:updateWarrantsList')

AddEventHandler('esx_sheriffmdt:askTabOpenResponse', function(status, force_veh_search, plate_number)
	if status == true then
...
RegisterNetEvent('esx_sheriffmdt:askTabOpenResponse')
      ...
      if (IsControlJustPressed(0, 182)) and GetLastInputMethod( 0 ) and tabEnabled == false then
				if server_requested == false then
					...
					if IsPedInAnyVehicle(GetPlayerPed(-1),true) and GetSeatPedIsTryingToEnter(GetPlayerPed(-1)) == -1 or GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) or GetPedInVehicleSeat(veh, 0) == GetPlayerPed(-1) or nearPC() then
						TriggerServerEvent('esx_sheriffmdt:askTabOpen', false, 'none')
						...
						if IsAnyVehicleNearPoint(coords.x,  coords.y,  coords.z,  5.0) then
							...
							TriggerServerEvent('esx_sheriffmdt:askTabOpen', true, all_trim(plate_number))
							server_requested = true
						end
					end
				end
...
RegisterCommand('mdt', function(...)
	...
	if plyData and plyData.job and plyData.job.name == "sheriff" then
		TriggerServerEvent('esx_sheriffmdt:askTabOpen', false, 'none')
		server_requested = true
	end
end)