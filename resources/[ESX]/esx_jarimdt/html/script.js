	...
	window.addEventListener('message', function(event) {		
        if ( event.data.type == 'GUI_pmobilet' ) {					
			if ( event.data.StatusJS == true ) {
				...
				$.post('http://esx_jarimdt/esx_jarimdt:GetWarrantsClient', JSON.stringify({
					page: 1
				}));
			}
			if ( event.data.ForceVehSearch == true ) {
				...
				$.post('http://esx_jarimdt/esx_jarimdt:SearchVehicleClient', JSON.stringify({
			...
			if ( event.data.report_added == true ) {
				...
				document.getElementById("notify_text").innerHTML = 'New Data has been sent';
				$('#notify_box').show();
			}
        }
    });
