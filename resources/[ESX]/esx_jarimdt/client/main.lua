...
RegisterNUICallback('esx_jarimdt:GetWarrantsClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:getWarrants', data.page)
end)

RegisterNUICallback('esx_jarimdt:GetBolosClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:getBolos', data.page)
end)

RegisterNUICallback('esx_jarimdt:GetArrestsClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:getArrests', data.page)
end)

RegisterNUICallback('esx_jarimdt:NewReportClient', function(data, cb)
	if data.report_type ~= '' and data.target ~= '' and data.desc ~= '' and data.charges ~= '' then
		TriggerServerEvent('esx_jarimdt:NewReportServer', data.report_type, data.target, data.desc, data.charges)
	end
end)

RegisterNUICallback('esx_jarimdt:CloseTab', function(data, cb)
	SendNUIMessage({
		type = 'GUI_pmobilet',
		StatusJS = false
	})
...
RegisterNUICallback('esx_jarimdt:SearchPersonClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:SearchPersonServer', data.fname, data.sname)
end)

RegisterNUICallback('esx_jarimdt:SearchVehicleClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:SearchVehicleServer', data.plate)
end)

RegisterNetEvent('esx_jarimdt:updateBolosList')
AddEventHandler('esx_jarimdt:updateBolosList', function(bolos_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
...
RegisterNetEvent('esx_jarimdt:updateArrestsList')
AddEventHandler('esx_jarimdt:updateArrestsList', function(arrests_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
	local design_shit = 'sui-row'
...
AddEventHandler('esx_jarimdt:NewReportResponse', function()
	SendNUIMessage({
		type = 'GUI_pmobilet',
		report_added = true
	})
end)
RegisterNetEvent('esx_jarimdt:NewReportResponse')

AddEventHandler('esx_jarimdt:SearchPersonFinish', function(player_name, properties, licenses, job, bills, has_warrants)
	local html_data = ''
...
RegisterNetEvent('esx_jarimdt:SearchPersonFinish')

AddEventHandler('esx_jarimdt:SearchVehicleFinish', function(veh_plate, veh_model, veh_owner)
	local html_data = ''
...
RegisterNetEvent('esx_jarimdt:SearchVehicleFinish')

AddEventHandler('esx_jarimdt:finishReportDelete', function(type_kek)
	if type_kek == 'warrants' then
		TriggerServerEvent('esx_jarimdt:getWarrants', 1)
	elseif type_kek == 'bolos' then
		TriggerServerEvent('esx_jarimdt:getBolos', 1)
	elseif type_kek == 'arrests' then
		TriggerServerEvent('esx_jarimdt:getArrests', 1)
	end
end)
RegisterNetEvent('esx_jarimdt:finishReportDelete')

RegisterNUICallback('esx_jarimdt:DeleteReportClient', function(data, cb)
	TriggerServerEvent('esx_jarimdt:DeleteReportServer', data.id, data.type)
end)

AddEventHandler('esx_jarimdt:updateWarrantsList', function(warrants_table, pagenum, total_pages, user_names, id_list)
	local html_list = ''
	local design_shit = 'sui-row'
...
RegisterNetEvent('esx_jarimdt:updateWarrantsList')

AddEventHandler('esx_jarimdt:askTabOpenResponse', function(status, force_veh_search, plate_number)
	if status == true then
...
RegisterNetEvent('esx_jarimdt:askTabOpenResponse')
      ...
      if (IsControlJustPressed(0, 182)) and GetLastInputMethod( 0 ) and tabEnabled == false then
				if server_requested == false then
					...
					if IsPedInAnyVehicle(GetPlayerPed(-1),true) and GetSeatPedIsTryingToEnter(GetPlayerPed(-1)) == -1 or GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) or GetPedInVehicleSeat(veh, 0) == GetPlayerPed(-1) or nearPC() then
						TriggerServerEvent('esx_jarimdt:askTabOpen', false, 'none')
						...
						if IsAnyVehicleNearPoint(coords.x,  coords.y,  coords.z,  5.0) then
							...
							TriggerServerEvent('esx_jarimdt:askTabOpen', true, all_trim(plate_number))
							server_requested = true
						end
					end
				end
...
RegisterCommand('emspatient', function(...)
	local plyData = ESX.GetPlayerData()
	if plyData and plyData.job and plyData.job.name == "ambulance" or "ambulance3" then
		TriggerServerEvent('esx_jarimdt:askTabOpen', false, 'none')
		server_requested = true
	end
end)