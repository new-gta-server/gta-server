...
AddEventHandler('montrer:permis', function(id)
    ...
    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'permis')
    if distance ~= -1 and distance <= 3.0 then
        TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), 'permis')                                
    else
        ESX.ShowNotification("Aucun joueur à proximité")
    end
end)
