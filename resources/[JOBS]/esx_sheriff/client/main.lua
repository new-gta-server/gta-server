    ...
    while messagenotfinish do
      ...
      DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "", "", "", "", 30)
        ...
        if (GetOnscreenKeyboardResult()) then
           ...
           TriggerServerEvent('esx_sheriffjob:annonce',result)
...
Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)    
                ...
                DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~b~ Sheriff ~d~", 255, 255, 255, 255, 1, 0)
...
function OpenSaisiMenu(station)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'saisi', {
		...
		if data.current.value == 'get_stock1' then
			OpenGetStocksMenu1()
		end
		if data.current.value == 'get_weapon' then
          OpenGetWeaponMenu()
...
function OpensheriffActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sheriff_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job and PlayerData.job.name == 'sheriff' and PlayerData.job.grade_name == 'NTM'  then
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(player), 'society_sheriff', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_sheriffjob:getStockItem1', itemName, count)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_sheriffjob:putStockItems1', itemName, count)
...
AddEventHandler('esx_sheriffjob:getarrested', function(playerheading, playercoords, playerlocation)
	...
	Citizen.Wait(3760)
	IsHandcuffed = true
	TriggerEvent('esx_sheriffjob:handcuff1')
...
RegisterNetEvent('esx_sheriffjob:handcuff1')
AddEventHandler('esx_sheriffjob:handcuff1', function()
...
RegisterNetEvent('esx_sheriffjob:annonce')
AddEventHandler('esx_sheriffjob:annonce', function(text)
    texteafiche = text
    affichenews = true
...
RegisterNetEvent('esx_sheriffjob:annoncestop')
AddEventHandler('esx_sheriffjob:annoncestop', function()
    affichenews = false
