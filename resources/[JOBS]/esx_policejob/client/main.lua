...
Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)    
                ...
                DrawAdvancedTextCNN(0.588, 0.14, 0.005, 0.0028, 0.8, "~b~ LSPD ~d~", 255, 255, 255, 255, 1, 0)
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job and PlayerData.job.name == 'police' and PlayerData.job.grade_name == 'NTGD'  then
			...
			if not DoesEntityExist(vehicle) then
				...
				if Config.MaxInService == -1 then
					...
					local plate = 'LSPD' .. math.random(100, 900)
						...
						TriggerServerEvent('esx_vehiclelock:givekey', 'yes', plate) -- vehicle lock
					...
					ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
						if isInService then
							...
							ESX.Game.SpawnVehicle(model, vehicles[partNum].SpawnPoint, vehicles[partNum].Heading, function(vehicle)
								TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
								SetVehicleMaxMods(vehicle)
								 TriggerServerEvent('esx_vehiclelock:givekey', 'yes', vehicle) -- vehicle lock
							end)
						else
